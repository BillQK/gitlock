defmodule GitlockHolmesCore.Domain.Services.CoupledHotspotAnalysisTest do
  ...
  describe "detect_combined/2" do
    test "identifies coupled files that are also hotspots" do
      ...
      if Enum.empty?(result) do
        ...
        hotspots =
          GitlockHolmesCore.Domain.Services.HotspotDetection.detect_hotspots(
            all_commits,
            complexity_metrics
          )

        couplings =
          GitlockHolmesCore.Domain.Services.CouplingDetection.detect_couplings(
            all_commits,
            # min_coupling 
            1.0,
            # min_windows
