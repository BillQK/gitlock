defmodule GitlockWorkflows.Runtime.Nodes.LogicTest do
  ...
  alias GitlockWorkflows.Runtime.Nodes.Logic.{Filter, Sort, Limit, If, Switch, Merge, Aggregate, RemoveDuplicates, GroupBy}
  ...
  describe "Filter" do
    test "keeps items matching eq condition" do
      {:ok, out} = Filter.execute(%{items: @hotspots}, %{"field" => "risk_factor", "operator" => "eq", "value" => "high"}, %{})
    ...
    test "numeric gt comparison" do
      {:ok, out} = Filter.execute(%{items: @hotspots}, %{"field" => "revisions", "operator" => "gt", "value" => "25"}, %{})
      assert length(out.kept) == 3
      assert Enum.all?(out.kept, &(&1.revisions > 25))
    end

    test "contains string matching" do
      {:ok, out} = Filter.execute(%{items: @hotspots}, %{"field" => "entity", "operator" => "contains", "value" => "core"}, %{})
      assert length(out.kept) == 1
      assert hd(out.kept).entity == "lib/core.ex"
    end

    test "empty field returns error" do
      assert {:error, _} = Filter.execute(%{items: @hotspots}, %{"field" => "", "operator" => "eq", "value" => "x"}, %{})
    end
  end
  ...
  describe "Sort" do
    test "sorts ascending by numeric field" do
      {:ok, out} = Sort.execute(%{items: @hotspots}, %{"field" => "revisions", "direction" => "asc"}, %{})
      revisions = Enum.map(out.items, & &1.revisions)
      assert revisions == Enum.sort(revisions)
    end

    test "sorts descending" do
      {:ok, out} = Sort.execute(%{items: @hotspots}, %{"field" => "risk_score", "direction" => "desc"}, %{})
  ...
  describe "IF" do
    test "splits items into true/false branches" do
      {:ok, out} = If.execute(%{items: @hotspots}, %{"field" => "risk_factor", "operator" => "eq", "value" => "high"}, %{})
      assert length(out.true) == 2
      assert length(out.false) == 3
    end

    test "numeric gte condition" do
      {:ok, out} = If.execute(%{items: @hotspots}, %{"field" => "revisions", "operator" => "gte", "value" => "30"}, %{})
    ...
    test "is_empty check" do
      ...
      {:ok, out} = If.execute(%{items: items}, %{"field" => "value", "operator" => "is_empty"}, %{})
      assert length(out.true) == 2
  ...
  describe "Switch" do
    test "routes items to named cases" do
      {:ok, out} = Switch.execute(%{items: @hotspots}, %{"field" => "risk_factor", "cases" => "high,medium,low"}, %{})
      assert length(out.case_0) == 2  # high
      assert length(out.case_1) == 2  # medium
      assert length(out.case_2) == 1  # low
      assert out.default == []
    end

    test "unmatched items go to default" do
      {:ok, out} = Switch.execute(%{items: @hotspots}, %{"field" => "risk_factor", "cases" => "critical"}, %{})
      assert out.case_0 == []
  ...
  describe "Aggregate" do
    ...
    test "computes sum and avg" do
      {:ok, out} = Aggregate.execute(%{items: @hotspots}, %{"operations" => "sum:revisions,avg:revisions"}, %{})
      assert out.result["sum_revisions"] == 155
      assert_in_delta out.result["avg_revisions"], 31.0, 0.01
    end

    test "computes min and max" do
      {:ok, out} = Aggregate.execute(%{items: @hotspots}, %{"operations" => "min:risk_score,max:risk_score"}, %{})
      assert out.result["min_risk_score"] == 1.2
      assert out.result["max_risk_score"] == 8.5
    end

    test "multiple operations in one call" do
      {:ok, out} = Aggregate.execute(%{items: @hotspots}, %{"operations" => "count,sum:revisions,max:risk_score"}, %{})
      assert out.result["count"] == 5
      assert out.result["sum_revisions"] == 155
      assert out.result["max_risk_score"] == 8.5
    end
  ...
  describe "NodeCatalog integration" do
    test "all logic nodes are in the catalog" do
      ...
      for expected <- ~w(filter sort limit if_condition switch merge aggregate remove_duplicates group_by)a do
