defmodule GitlockMCP.Server do
  ...
  def init(_client_info, frame) do
    {:ok,
     ...
     |> register_tool("gitlock_assess_file",
       input_schema: %{
         file_path:
           {:required, :string, description: "Path to the file relative to repo root"}
       },
       description:
     ...
     |> register_tool("gitlock_hotspots",
       input_schema: %{
         directory: {:optional, :string, description: "Directory to filter (e.g. lib/payments). Omit for entire repo."},
     ...
     |> register_tool("gitlock_find_coupling",
       input_schema: %{
         file_path:
           {:required, :string, description: "Path to the file relative to repo root"},
         min_coupling:
           {:optional, :integer, description: "Minimum coupling percentage to include (default: 30)"}
       },
       description:
  ...
  defp format_assess_file(a) do
    coupled = Enum.map_join(a.coupled_files, "\n", fn c ->
      "  - #{c.file} (#{c.coupling_pct}% co-change)"
    end)

    ownership = if a.ownership do
      o = a.ownership
  ...
  defp format_review(r) do
    files = Enum.map_join(r.file_assessments, "\n", fn a ->
      "- #{a.file} — risk: #{a.risk_score} (#{a.risk_level})"
    end)

    missing = if length(r.missing_coupled_files) > 0 do
    ...
    reviewers = if length(r.suggested_reviewers) > 0 do
  ...
  defp format_summary(r) do
    ...
    areas = if length(r.riskiest_areas) > 0 do
      "\n\nRiskiest areas:\n" <>
